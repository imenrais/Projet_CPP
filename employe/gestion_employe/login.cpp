...
void login::on_loginButton_clicked()
{
    this->loggedIn = Login(ui->usernameBox->text(), ui->passwordBox->text());
        ...
        if(this->loggedIn)
        {
            ...
            ui->winStack->setCurrentIndex(2);// if login succeful enter the next page

            if(serial->isWritable()){
                serial->write("1");//send to arduino
...
void login::on_logoutButton_clicked()
{
    if(QMessageBox::Yes == QMessageBox(QMessageBox::Question,
                                           "Login System", "Are you sure you want to visit app?",
                                           QMessageBox::Yes|QMessageBox::No).exec())
        {
            this->loggedIn = false;
            ui->passwordBox->setText("");
            ui->loginLabel->setText("application!");
            ui->winStack->setCurrentIndex(0);
            hide();
            mainwindow= new MainWindow(this);
            mainwindow->show();
...
void login::on_arduino_clicked()
{
    ...
    if(serial->isReadable())
    {
      ...
      if(data.contains('0'))
         {ui->loginLabel->setText("identification echou");
      qDebug()<<"login echou";
      QMessageBox::information(nullptr, QObject::tr("arduino is closed"),
                  QObject::tr("login par  RIFD failed.\n"
